            ...
            <div style={{ display: 'flex', height: 'calc(100vh - 36px)' }}>
                ...
                <div style={{ width: 320, flexShrink: 0, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                    ...
                    <div style={{ borderBottom: `1px solid ${C.border}`, padding: '12px 0' }}>
                        <div style={{ fontSize: 12, color: C.sub, letterSpacing: '0.14em', padding: '0 16px', marginBottom: 8 }}>DETECTION LAYERS</div>
                                ...
                                >
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            <span style={{ fontSize: 12, color: active ? C.acid : C.muted, border: `1px solid ${active ? C.acid : C.muted}`, padding: '1px 4px', fontWeight: 700 }}>
                                                {short}
                                            </span>
                                            <span style={{ fontSize: 13, color: active ? C.text : C.sub, fontWeight: active ? 700 : 400 }}>{label}</span>
                                        </div>
                                        <span style={{ fontSize: 12, color: active ? C.acid : C.red, fontWeight: 700 }}>
                                            {active ? '[ LIVE ]' : '[ OFFLINE ]'}
                                        </span>
                                    </div>
                    ...
                    {enginesStatus && (
                        <div style={{ borderBottom: `1px solid ${C.border}`, padding: '12px 0' }}>
                            <div style={{ fontSize: 12, color: C.sub, letterSpacing: '0.14em', padding: '0 16px', marginBottom: 8 }}>ENGINE FLEET</div>
                            {Object.entries(enginesStatus.engines).map(([key, eng]) => (
                                <div key={key} style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                        <span style={{
                                            fontSize: 11, border: `1px solid ${eng.loaded ? C.acid : C.muted}`,
                                            color: eng.loaded ? C.acid : C.muted, padding: '1px 3px', fontWeight: 700,
                                        }}>
                                            {ENGINE_SHORT[key] ?? key.slice(0, 3).toUpperCase()}
                                        </span>
                                        <span style={{ fontSize: 12, color: eng.loaded ? C.text : C.sub }}>{eng.label}</span>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                        {eng.checkpoint !== undefined && eng.checkpoint !== null && (
                                            <span style={{ fontSize: 11, color: eng.checkpoint ? C.acid : C.amber }}>
                                                {eng.checkpoint ? '✓ PT' : '✗ PT'}
                                            </span>
                                        )}
                                        <span style={{ fontSize: 11, color: eng.loaded ? C.acid : C.red, fontWeight: 700 }}>
                                            {eng.loaded ? 'LIVE' : 'OFF'}
                                        </span>
                                    </div>
                                </div>
                    ...
                    {/* Model metadata */}
                    <div style={{ borderBottom: `1px solid ${C.border}`, padding: '12px 0' }}>
                        <div style={{ fontSize: 12, color: C.sub, letterSpacing: '0.14em', padding: '0 16px', marginBottom: 8 }}>MODEL METADATA</div>
                        {[
                            { label: 'TYPE',     value: modelStats.model_type },
                            { label: 'TRAINED',  value: modelStats.last_trained },
                            { label: 'SIZE',     value: modelStats.file_size },
                        ].map(({ label, value }) => (
                            <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 16px', fontSize: 13 }}>
                    ...
                    <div style={{ padding: '12px 0' }}>
                        <div style={{ fontSize: 12, color: C.sub, letterSpacing: '0.14em', padding: '0 16px', marginBottom: 8 }}>
                            DRIFT MONITOR
                            {driftData?.drift_alert && (
                        ...
                        {!driftData ? (
                            <div style={{ padding: '4px 16px', fontSize: 13, color: C.sub, lineHeight: 1.7 }}>
                            ...
                            <>
                                ...
                                {driftData.drift_alert && (
                                    <div style={{ margin: '8px 16px', padding: '8px 10px', background: 'rgba(255,140,0,0.08)', border: `1px solid rgba(255,140,0,0.25)`, fontSize: 12, color: '#FFBA60', lineHeight: 1.6 }}>
                    ...
                    <div style={{ marginTop: 'auto', borderTop: `1px solid ${C.border}`, padding: 16 }}>
                        <div style={{ fontSize: 12, color: C.sub, letterSpacing: '0.12em', marginBottom: 8 }}>MODEL UPDATES</div>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    ...
                    <div style={{
                        ...
                        padding: '0 16px', fontSize: 13, color: C.sub, letterSpacing: '0.1em',
                    }}>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
                        {[
                            ...
                            <div key={short} style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                                    <span style={{
                                        fontSize: 12, border: `1px solid ${active ? C.acid : C.muted}`,
                                        color: active ? C.acid : C.muted, padding: '2px 6px', fontWeight: 700,
                                    }}>{short}</span>
                                    <span style={{ fontSize: 15, fontWeight: 700, color: active ? C.text : C.sub }}>{label}</span>
                                    <span style={{ marginLeft: 'auto', fontSize: 12, color: active ? C.acid : C.red, fontWeight: 700 }}>
                                        {active ? '● ACTIVE' : '○ OFFLINE'}
                                    </span>
                                </div>
                                <p style={{ margin: '0 0 8px 0', fontSize: 13, color: C.sub, lineHeight: 1.7 }}>{how}</p>
                        ...
                        <div style={{ padding: '12px 16px', background: '#050505', border: `1px solid #1A1A1A`, fontSize: 12, color: C.sub, lineHeight: 1.8 }}>
