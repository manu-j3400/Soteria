...
function StatusCell({ label, value, accent }: { label: string; value: string | number; accent?: string }) {
  return (
    <div className="flex items-center gap-3 px-5 h-full" style={{ borderRight: `1px solid ${C.dim}` }}>
      <span className="text-[13px] tracking-[0.18em]" style={{ color: C.muted }}>{label}</span>
      <span className="text-[15px] font-bold tracking-[0.1em]" style={{ color: accent ?? C.text, ...MONO }}>{value}</span>
    </div>
  );
}
...
function MetricRow({ label, value, color }: { label: string; value: number | string; color?: string }) {
  return (
    <div className="flex items-center justify-between px-5 py-2" style={{ borderBottom: `1px solid ${C.faint}` }}>
      <span className="text-[14px] tracking-[0.12em]" style={{ color: C.muted }}>{label}</span>
      <span className="text-[17px] font-bold tabular-nums" style={{ color: color ?? C.text, ...MONO }}>{value}</span>
    </div>
  );
}
  ...
  return (
    <motion.div
      ...
      <span className="w-2 h-2 flex-shrink-0 flex items-center justify-center" style={{ ...MONO }}>
        {selected
          ? <span className="text-[14px] font-bold" style={{ color: C.acid }}>✓</span>
          : <span style={{ color: C.faint }}>·</span>}
      </span>

      {/* ID */}
      <span className="text-[13px] tabular-nums w-10 flex-shrink-0" style={{ color: C.muted, ...MONO }}>
        #{scan.id}
      </span>

      {/* Age */}
      <span className="text-[13px] w-8 flex-shrink-0 tabular-nums" style={{ color: C.muted, ...MONO }}>
        {age}
      </span>

      {/* Risk badge */}
      <span className="text-[13px] font-bold tracking-widest w-11 flex-shrink-0" style={{ color: rc, ...MONO }}>
        [{riskLabel(scan.risk_level)}]
      </span>

      {/* Language */}
      <span className="text-[13px] w-14 flex-shrink-0 truncate" style={{ color: C.sub, ...MONO }}>
        {scan.language}
      </span>
      ...
      <div className="flex-1 flex items-center gap-2 min-w-0">
        ...
        <span className="text-[13px] tabular-nums flex-shrink-0" style={{ color: C.muted, ...MONO }}>
          {scan.confidence}%
        </span>
      </div>
    </motion.div>
  ...
  return (
    <div className="h-screen flex overflow-hidden" style={{ background: '#030712', ...MONO, color: C.text }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden" style={{ marginLeft: '12rem' }}>
        ...
        {/* ══ STATUS STRIP ══════════════════════════════════════════════════ */}
        <header className="flex items-center h-11 flex-shrink-0 sticky top-0 z-40"
        ...
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex flex-col w-64 flex-shrink-0 overflow-y-auto"
            ...
            {/* Score */}
            <PanelHeader>
              <span className="text-[14px] font-bold tracking-[0.18em]" style={{ color: C.muted }}>SECURITY POSTURE</span>
              {!loading && (
                <span className="text-[14px] font-bold tracking-[0.06em]" style={{ color: C.sub }}>
                  {score}/100
                </span>
              )}
            </PanelHeader>
            ...
            <div className="px-5 py-5 flex items-center gap-5"
              ...
              <div className="flex flex-col gap-1.5">
                <div>
                  <div className="text-[13px] tracking-[0.14em]" style={{ color: C.muted }}>7-DAY TREND</div>
                  <div className="mt-1">
            ...
            <div style={{ borderBottom: `1px solid ${C.dim}` }}>
              <div className="px-5 pt-3 pb-1">
                <span className="text-[13px] tracking-[0.2em] font-bold" style={{ color: C.faint }}>SCAN STATS</span>
              </div>
            ...
            <div style={{ borderBottom: `1px solid ${C.dim}` }}>
              <div className="px-5 pt-3 pb-1">
                <span className="text-[13px] tracking-[0.2em] font-bold" style={{ color: C.faint }}>RISK DISTRIBUTION</span>
              </div>
              {(['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'] as const).map(r => {
                ...
                return (
                  <div key={r} className="flex items-center gap-3 px-5 py-1.5"
                    ...
                    <span className="text-[13px] font-bold tracking-widest w-14 flex-shrink-0"
                      style={{ color: rc, ...MONO }}>{r === 'CRITICAL' ? 'CRIT' : r}</span>
                    <AsciiBar value={count} max={maxRisk} color={rc} />
                    <span className="text-[14px] font-bold tabular-nums ml-auto"
            ...
            {!loading && data && Object.keys(data.languages).length > 0 && (
              <div style={{ borderBottom: `1px solid ${C.dim}` }}>
                <div className="px-5 pt-3 pb-1">
                  <span className="text-[13px] tracking-[0.2em] font-bold" style={{ color: C.faint }}>LANGUAGES</span>
                </div>
                {Object.entries(data.languages).slice(0, 6).map(([lang, count]) => {
                  ...
                  return (
                    <div key={lang} className="flex items-center gap-3 px-5 py-1.5"
                      style={{ borderBottom: `1px solid ${C.faint}` }}>
                      <span className="text-[13px] font-bold w-16 truncate flex-shrink-0"
            ...
            {contrib && (
              <div style={{ borderBottom: `1px solid ${C.dim}` }}>
                <div className="px-5 pt-3 pb-1 flex items-center justify-between">
                  <span className="text-[13px] tracking-[0.2em] font-bold" style={{ color: C.faint }}>MY CONTRIBUTIONS</span>
                  {contrib.total > 0 && (
                    <span className="text-[12px]" style={{ color: C.sub, ...MONO }}>{contrib.total} SAMPLES</span>
                  )}
                </div>
                {contrib.total === 0 ? (
                  <div className="px-5 py-2 text-[13px]" style={{ color: C.sub, ...MONO }}>
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-3 px-5 py-1.5" style={{ borderBottom: `1px solid ${C.faint}` }}>
                      <span className="text-[13px] font-bold w-14 flex-shrink-0" style={{ color: C.red, ...MONO }}>THREAT</span>
                      <AsciiBar value={contrib.malicious} max={contrib.total} color={C.red} />
                      <span className="text-[13px] tabular-nums ml-auto" style={{ color: C.red, ...MONO }}>{contrib.malicious}</span>
                    </div>
                    <div className="flex items-center gap-3 px-5 py-1.5" style={{ borderBottom: `1px solid ${C.faint}` }}>
                      <span className="text-[13px] font-bold w-14 flex-shrink-0" style={{ color: C.acid, ...MONO }}>CLEAN</span>
                      <AsciiBar value={contrib.clean} max={contrib.total} color={C.acid} />
                      <span className="text-[13px] tabular-nums ml-auto" style={{ color: C.acid, ...MONO }}>{contrib.clean}</span>
                    </div>
                    ...
                    {contrib.by_language.slice(0, 4).map(({ language, count }) => (
                      <div key={language} className="flex items-center gap-3 px-5 py-1" style={{ borderBottom: `1px solid ${C.faint}` }}>
                        <span className="text-[12px] font-bold w-14 truncate flex-shrink-0" style={{ color: C.sub, ...MONO }}>{language}</span>
                        <AsciiBar value={count} max={contrib.total} color="#3A3A3A" />
                        <span className="text-[12px] tabular-nums ml-auto" style={{ color: C.sub, ...MONO }}>{count}</span>
                      </div>
                    ))}
                    {contrib.last_collected && (
                      <div className="px-5 py-1.5 text-[12px]" style={{ color: C.sub, ...MONO }}>
            ...
            {drift && (
              <div style={{ borderBottom: `1px solid ${C.dim}` }}>
                <div className="px-5 pt-3 pb-1 flex items-center justify-between">
                  <span className="text-[13px] tracking-[0.2em] font-bold" style={{ color: C.faint }}>MODEL DRIFT</span>
                  <span
                ...
                {drift.status === 'insufficient_data' ? (
                  ...
                  <>
                    <div className="flex items-center justify-between px-5 py-1.5" style={{ borderBottom: `1px solid ${C.faint}` }}>
                      <span className="text-[13px] tracking-[0.1em]" style={{ color: C.muted }}>KL DIVERGENCE</span>
                    ...
                    <div className="flex items-center justify-between px-5 py-1.5" style={{ borderBottom: `1px solid ${C.faint}` }}>
                      <span className="text-[13px] tracking-[0.1em]" style={{ color: C.muted }}>BASELINE → RECENT</span>
                      <span className="text-[13px] tabular-nums" style={{ color: C.sub, ...MONO }}>
                        {(drift.baseline_mean ?? 0).toFixed(4)} → {(drift.recent_mean ?? 0).toFixed(4)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between px-5 py-1.5" style={{ borderBottom: `1px solid ${C.faint}` }}>
                      <span className="text-[13px] tracking-[0.1em]" style={{ color: C.muted }}>SAMPLES</span>
                      <span className="text-[13px] tabular-nums" style={{ color: C.sub, ...MONO }}>
                        {drift.total_samples} (base: {drift.baseline_samples}, win: {drift.recent_window})
                      </span>
                    </div>
                    {drift.drift_alert && (
                      <div className="px-5 py-1.5 text-[13px] font-bold tracking-widest" style={{ color: C.red, ...MONO }}>
                        ⚠ DRIFT ALERT — KL &gt; 0.5
                      </div>
                    )}
                  </>
            ...
            <div className="px-4 py-4 flex flex-col gap-2 mt-auto">
              <div className="text-[13px] tracking-[0.2em] px-1 mb-1" style={{ color: C.faint }}>QUICK NAVIGATE</div>
              {[
                ...
                <button key={to} onClick={() => navigate(to)}
                  className="flex items-center justify-between px-3 py-2 text-[14px] font-bold tracking-[0.1em] transition-all duration-100 group"
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <PanelHeader>
              <div className="flex items-center gap-2">
                <span className="text-[14px] font-bold tracking-[0.18em]" style={{ color: C.muted }}>SCAN HISTORY</span>
              ...
              <div className="flex items-center gap-1">
                ...
                {compareIds.length === 2 && (
                  <button onClick={handleCompare} disabled={compareLoading}
                    className="flex items-center gap-1.5 px-3 py-1 text-[13px] font-bold tracking-widest transition-all duration-100"
                ...
                {compareIds.length > 0 && (
                  <button onClick={() => { setCompareIds([]); setCompareResult(null); }}
                    className="px-2 py-1 text-[13px] transition-colors duration-100"
              ...
              {showWebhook && (
                <motion.div
                  ...
                  <div className="flex items-center gap-3 px-5 py-3">
                    <span className="text-[13px] tracking-[0.14em] flex-shrink-0" style={{ color: C.muted }}>
                      WEBHOOK URL
                    </span>
                    <input
                      type="url"
                      ...
                      className="flex-1 bg-transparent text-[14px] outline-none px-3 py-1.5 font-bold"
                      style={{
                        ...MONO,
                    ...
                    <button onClick={handleSaveWebhook}
                      className="flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-bold tracking-widest transition-all duration-100"
                      style={{
            ...
            <AnimatePresence>
              {(compareResult || compareError) && (
                <motion.div
                  ...
                  className="overflow-hidden flex-shrink-0"
                  style={{ borderBottom: `1px solid ${C.dim}` }}>
                  <div className="px-5 py-3 text-[14px]"
            ...
            <div className="flex items-center gap-3 px-5 py-2 flex-shrink-0"
              ...
              <span className="text-[12px] tracking-[0.2em] w-10 flex-shrink-0" style={{ color: C.faint }}>ID</span>
              <span className="text-[12px] tracking-[0.2em] w-8 flex-shrink-0"  style={{ color: C.faint }}>AGE</span>
              <span className="text-[12px] tracking-[0.2em] w-11 flex-shrink-0" style={{ color: C.faint }}>RISK</span>
              <span className="text-[12px] tracking-[0.2em] w-14 flex-shrink-0" style={{ color: C.faint }}>LANG</span>
              <span className="text-[12px] tracking-[0.2em] flex-1"             style={{ color: C.faint }}>CONFIDENCE</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="flex flex-col justify-center items-start px-5 py-10">
                  <div className="text-[15px]" style={{ color: C.muted, ...MONO }}>
                    <span style={{ color: C.acid }}>soteria@scanner</span>
                    <span style={{ color: C.sub }}>:~$</span>
                    <span className="ml-2 text-[#444]">ls -la ./scan-history</span>
                  </div>
                  <div className="mt-2 text-[15px]" style={{ color: C.faint, ...MONO }}>
                    total 0
                  </div>
                  <div className="mt-3 text-[15px]" style={{ color: C.muted, ...MONO }}>
                    No scans found. Run your first scan to begin.
                  </div>
                  <button onClick={() => navigate('/scanner')}
                    className="mt-5 flex items-center gap-2 px-4 py-2 text-[14px] font-bold tracking-[0.14em] transition-all duration-100"
            ...
            <div className="flex items-center gap-5 px-5 py-2 flex-shrink-0"
              ...
              <span className="text-[13px]" style={{ color: C.faint, ...MONO }}>
                {data?.recent_scans?.length ?? 0} RECORDS
              </span>
              <span className="text-[13px]" style={{ color: C.faint, ...MONO }}>
                SELECT 2 TO COMPARE
              </span>
              <span className="text-[13px] ml-auto" style={{ color: C.faint, ...MONO }}>
                KYBER ENGINE v3.0
              </span>
            </div>
          </div>
